...
#endif
{
    ...
    */
    ...
    if (QBreakpadInstance.appCrashCallback() != 0 && QBreakpadInstance.appContext() != 0){
        QBreakpadInstance.appCrashCallback()(succeeded, path, QBreakpadInstance.appContext());
    }else{
        qDebug()<<"QBreakpadInstance.appCrashCallback() is null or QBreakpadInstance.appContext() is null";
    }
    qDebug() << "End";
    return succeeded;
}
...
void QBreakpadHandler::setAppCrashCallback(AppCrashCallback callback,void* ctx)
{
    m_callback = callback;
    m_ctx = ctx;
}

AppCrashCallback QBreakpadHandler::appCrashCallback()
{
    return m_callback;
}
void* QBreakpadHandler::appContext()
{
    return m_ctx;
}
